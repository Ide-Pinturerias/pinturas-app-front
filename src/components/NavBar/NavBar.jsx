...
function NavBar() {
    ...
    const activeLinkStl = 'bg-primaryVisible'; // estado activo
            ...
            <div className="container mx-auto flex justify-between items-center">
                ...
                <nav className={`${isMenuOpen ? 'block' : 'hidden'} md:block absolute md:relative top-full left-0 w-full md:w-auto bg-white md:bg-transparent z-20`}>
                    <ul className="flex flex-col md:flex-row md:space-x-4 items-center text-center">
                        {['/products', '/blog', '/about', '/contact', '/cart', '/account'].map((path, index) => {
                            const isActive = location.pathname === path;
                            return (
                                <li key={index}>
                                    <Link to={path} className={`${linkStl} ${isActive ? activeLinkStl : ''}`} onClick={() => setIsMenuOpen(false)}>
                                        {path === '/cart' ? <><Cart size={'1rem'} /> Carrito</> : null}
                                        {path === '/account' ? <><UserIcon size={'0.75rem'} /> Cuenta</> : null}
                                        {['/products', '/blog', '/about', '/contact'].includes(path) ? path.substring(1) : null}
                                    </Link>
                                </li>
                            );
                        })}
